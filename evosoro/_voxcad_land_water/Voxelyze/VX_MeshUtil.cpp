...
CVX_MeshUtil::CVX_MeshUtil(void)
{
	...
	pSimView = NULL;
	pFEA = NULL;
	qhullVolumeStart = 0.0;
	qhullVolumeEnd = 0.0;
	robotVolumeStart = 0.0;
	robotVolumeEnd = 0.0;
...
double CVX_MeshUtil::invokeQhull()
{
	...
	while(attempts++ < maxAttempts)
	{
		...
		std::string qhullTmpFile = pSim->getQhullTmpFile();
		// Achieving basePath from qhulltmpfile, to invoke qhull with absolute path (required when dispatching voxelyze on different nodes on a cluster)
		std::string basePath = qhullTmpFile.substr(0,qhullTmpFile.find_last_of("/"));

		qhullCmd1 = basePath+"/../qhull FS TI "+qhullTmpFile+" | sed 1d | tr -s \\ | cut -f3 -d \\ "; // invoke qhull with input file QhullTmpFile and get the volume of the convex hull
...
double CVX_MeshUtil::computeShapeComplexity(std::string tmpFileName)
{
	...
	// Time to print angleExcesses to file 'tmpFileName' (input to curvatureEntropy.py)
	...
	// Now going to call python program that will compute the entropy of the values in tmpFileName and override the file with the computed entropy
	std::string qhullTmpFile = pSim->getQhullTmpFile();

	// Achieving basePath from qhulltmpfile, to invoke qhull with absolute path (required when dispatching voxelyze on different nodes on a cluster)
	std::string basePath = qhullTmpFile.substr(0,qhullTmpFile.find_last_of("/"));

	// Invoking external python script to compute the entropy	
	std::string command("python "+ basePath +"/../curvatureEntropy.py "+tmpFileName);
	...
	std::string command2("rm "+tmpFileName);	
