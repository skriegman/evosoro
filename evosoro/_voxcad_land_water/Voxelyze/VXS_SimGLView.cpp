...
void CVXS_SimGLView::Draw(int Selected, bool ViewSection, int SectionLayer)
{
	...
	// nac: set mesh area values for water
	// for (int i=0; i<pSim->NumVox(); i++) { std::cout << pSim->VoxArray[i].GetCurPos().x << std::endl;}
...
void CVXS_SimGLView::DrawFloor(void)
{
	...
	for (int i=-20; i <=30; i++){
		for (int j=-40; j <=60; j++){
			...
			glColor4d(0.8, 0.8, 0.8, 1.0);//glColor4d(1.0, 1.0, 1.0, 1.0);
